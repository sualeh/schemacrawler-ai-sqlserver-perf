...
class TestConnectionFunctions:
    ...
    @patch('schemacrawler_ai_sqlserver_perf.database.connection.DatabaseConfig.from_environment')
...
class TestIntegration:
    ...
    def test_full_environment_configuration_flow(self):
        ...
        with patch.dict(os.environ, env_vars, clear=True):
            with patch('schemacrawler_ai_sqlserver_perf.database.connection.pyodbc') as mock_pyodbc:
                mock_connection = MagicMock()
                mock_cursor = MagicMock()
                mock_cursor.fetchone.return_value = (1,)
                mock_connection.cursor.return_value = mock_cursor
